...
#ifndef HASH_JOIN_OPERATOR_CAT_TWO_PAYLOADS_H
#define HASH_JOIN_OPERATOR_CAT_TWO_PAYLOADS_H

#include "ConciseArrayTableTwoPayloads.h"
...
namespace query_processor {
    ...
    class HashJoinOperatorCATTwoPayloads : public HashJoinOperatorBase<TABLE_PARTITION_SIZE> {
        ...
        protected:
            ...
            // maps the column ids of the build pipe to the columns ids of the probe pipe
            uint32_t _buildBreakerPayloadColumnIdFirst;
            uint32_t _probePayloadColumnIdFirst;
            uint32_t _buildBreakerPayloadColumnIdSecond;
            uint32_t _probePayloadColumnIdSecond;
            std::shared_ptr<TemporaryColumn<TABLE_PARTITION_SIZE>> _buildSidePayloadColumnFirst;
            std::shared_ptr<TemporaryColumn<TABLE_PARTITION_SIZE>> _buildSidePayloadColumnSecond;
            ...
            class OverflowMaterializationTask : public basis::TaskBase {
                private:
                    ...
                    std::vector<typename ConciseArrayTableTwoPayloads<TABLE_PARTITION_SIZE>::OverflowResult>& _overflowResultsPartition;
                ...
                public:
                    OverflowMaterializationTask(
                        ...
                        std::vector<typename ConciseArrayTableTwoPayloads<TABLE_PARTITION_SIZE>::OverflowResult>& overflowResultsPartition,
                        ...
                        // maps the column ids of the build pipe to the columns ids of the probe pipe
                        uint32_t buildBreakerPayloadColumnIdFirst,
                        uint32_t probePayloadColumnIdFirst,
                        uint32_t buildBreakerPayloadColumnIdSecond,
                        uint32_t probePayloadColumnIdSecond,
                        std::shared_ptr<TemporaryColumn<TABLE_PARTITION_SIZE>> buildSidePayloadColumnFirst,
                        std::shared_ptr<TemporaryColumn<TABLE_PARTITION_SIZE>> buildSidePayloadColumnSecond,
                    ...
                    : basis::TaskBase(basis::OLAP_TASK_PRIO, sourceBatch->getNumaNode()),
                        ...
                        _buildBreakerPayloadColumnIdFirst(buildBreakerPayloadColumnIdFirst),
                        _probePayloadColumnIdFirst(probePayloadColumnIdFirst),
                        _buildBreakerPayloadColumnIdSecond(buildBreakerPayloadColumnIdSecond),
                        _probePayloadColumnIdSecond(probePayloadColumnIdSecond),
                        _buildSidePayloadColumnFirst(buildSidePayloadColumnFirst),
                        _buildSidePayloadColumnSecond(buildSidePayloadColumnSecond),
                    ...
                    void execute(){
                        ...
                        targetBatch->addNewColumnPartition(_buildSidePayloadColumnFirst);
                        targetBatch->addNewColumnPartition(_buildSidePayloadColumnSecond);
                        //
                        ...
                        // (2a) fill the first new column
                        {
                            ...
                            auto* tempPartition = tempPartitions[_probePayloadColumnIdFirst];
                            while(spaceLeft > 0 && startIndexPartition < _overflowResultsPartition.size()){
                                // determine iterator to the start of current overflow result
                                uint32_t startOffset = _overflowResultsPartition[startIndexPartition]._start;
                                // apply offset, but only for the first overflow result
                                ...
                                while(spaceLeft > 0 && startOffset < _overflowResultsPartition[startIndexPartition]._end){
                                      ...
                                      tempPartition->setEntry(targetBatchRowId, _overflowResultsPartition[startIndexPartition]._array->at(startOffset)._first);
                                      // decrement left space in this tuple
                                      spaceLeft--;
                                      // move iterator to the next probe result tuple
                                      startOffset++;
                        ...
                        // (2b) fill the second new column
                        {
                            first = true;
                            spaceLeft = Batch<TABLE_PARTITION_SIZE>::getMaximumSize();
                            startIndexPartition = _currentStartIndexPartition;
                            uint32_t targetBatchRowId = 0;
                            auto* tempPartition = tempPartitions[_probePayloadColumnIdSecond];
                            while(spaceLeft > 0 && startIndexPartition < _overflowResultsPartition.size()){
                                // determine iterator to the start of current overflow result
                                uint32_t startOffset = _overflowResultsPartition[startIndexPartition]._start;
                                // apply offset, but only for the first overflow result
                                ...
                                while(spaceLeft > 0 && startOffset < _overflowResultsPartition[startIndexPartition]._end){
                                      // copy the entry from the source column partition into the new target column partition
                                      tempPartition->setEntry(targetBatchRowId, _overflowResultsPartition[startIndexPartition]._array->at(startOffset)._second);
                                      // decrement left space in this tuple
                                      spaceLeft--;
                                      // move iterator to the next probe result tuple
                                      startOffset++;
        ...
        public:
            ...
            HashJoinOperatorCATTwoPayloads(
                ...
                for(uint32_t oldColumnId = 0; oldColumnId < columns.size(); ++oldColumnId) {
                    ...
                    if (it != _columnIdMappings.getMapping().end()) {
                        if (first) {
                            first = false;
                            _buildSidePayloadColumnFirst = columns[oldColumnId]._column;
                            _buildBreakerPayloadColumnIdFirst = oldColumnId;
                            _probePayloadColumnIdFirst = it->second;
                        } else {
                            _buildSidePayloadColumnSecond = columns[oldColumnId]._column;
                            _buildBreakerPayloadColumnIdSecond = oldColumnId;
                            _probePayloadColumnIdSecond = it->second;
                        }
                    }
                }
            }
            ...
            void pushLastJoin(std::shared_ptr<Batch<TABLE_PARTITION_SIZE>> batch, uint32_t workerId){ // a.k.a. probe
                ...
                batch->addNewColumnPartition(_buildSidePayloadColumnFirst);
                batch->addNewColumnPartition(_buildSidePayloadColumnSecond);
                ...
                for(uint32_t batchRowId=0; batchRowId<batch->getCurrentSize(); ++batchRowId){
                    ...
                    if(batch->isRowValid(batchRowId)){
                        ...
                        if (_hashTable.isInPartitionRange(key)) {
                            typename ConciseArrayTableTwoPayloads<TABLE_PARTITION_SIZE>::MainResult mainResultPartition;
                            // probe for the key
                            if (_hashTable.probe(key, mainResultPartition)) {
                                ...
                                while (mainResultPartition._start < mainResultPartition._end) {
                                    checksums[_probePayloadColumnIdFirst] += mainResultPartition._array->at(mainResultPartition._start)._first;
                                    checksums[_probePayloadColumnIdSecond] += mainResultPartition._array->at(mainResultPartition._start)._second;
                                    // update the iterator
                ...
                for (uint32_t columnId = 0; columnId < columns.size(); ++columnId) {
                    if (columns[columnId]._isProjectionColumn && columnId != _probePayloadColumnIdFirst && columnId != _probePayloadColumnIdSecond) {
            ...
            void pushInnerJoin(std::shared_ptr<Batch<TABLE_PARTITION_SIZE>> batch, uint32_t workerId){ // a.k.a. probe
                ...
                auto* probePipePayloadColumnFirstTempPartition = tempPartitions[_probePayloadColumnIdFirst];
                auto* probePipePayloadColumnSecondTempPartition = tempPartitions[_probePayloadColumnIdSecond];
                    ...
                    if(batch->isRowValid(batchRowId)){
                        ...
                        if (_hashTable.isInPartitionRange(key)) {
                            ...
                            if (_hashTable.probe(key, mainResultPartition)) {
                                ...
                                probePipePayloadColumnFirstTempPartition->setEntry(batchRowId, mainResultPartition._array->at(mainResultPartition._start)._first);
                                probePipePayloadColumnSecondTempPartition->setEntry(batchRowId, mainResultPartition._array->at(mainResultPartition._start)._second);
                                // update the iterator
                                ++mainResultPartition._start;
                    ...
                    for(uint64_t resultIndex=0; resultIndex<overflowResultsPartition.size(); resultIndex++){
                        ...
                        // in case the overflow result does not fit into the current batch
                            ...
                            overflowGroup->addTask(std::shared_ptr<OverflowMaterializationTask>(new(numaDistributionCounter++%basis::NUMA_NODE_COUNT) OverflowMaterializationTask(
                                ...
                                _buildBreakerPayloadColumnIdFirst,
                                _probePayloadColumnIdFirst,
                                _buildBreakerPayloadColumnIdSecond,
                                _probePayloadColumnIdSecond,
                                _buildSidePayloadColumnFirst,
                                _buildSidePayloadColumnSecond,
    ...
    //
    template <uint32_t TABLE_PARTITION_SIZE>
    class HashJoinLateOperatorCATTwoPayloads : public HashJoinOperatorCATTwoPayloads<TABLE_PARTITION_SIZE> {
        private:
        ...
        public:
            HashJoinLateOperatorCATTwoPayloads(
                const PipelineColumnsContainer<TABLE_PARTITION_SIZE>& currentPipeColumns, // since this is a flushing operator we need this information for base class
                PipelineColumnsContainer<TABLE_PARTITION_SIZE>& probeSideColumnsOnly, // necessary in expanding joins when we have to duplicate tuples
                ColumnIdMappingsContainer& columnIdMappings, // maps the columns ids of the build pipe to the columns ids of the probe pipe
                PipelineColumnsContainer<TABLE_PARTITION_SIZE>& buildPipeColumns, // column pointer needed in the probe/push function to add empty column partitions to the pushed batches
                ...
                RuntimeStatistics* runtimeStatistics
            ) : HashJoinOperatorCATTwoPayloads<TABLE_PARTITION_SIZE>(
            ...
            void buildHashTable(){
                ...
                this->_hashTable.initializePartitions(
                    ...
                    this->_buildBreakerPayloadColumnIdFirst,
                    this->_buildBreakerPayloadColumnIdSecond,
    ...
    //
    template <uint32_t TABLE_PARTITION_SIZE>
    class HashJoinEarlyBreakerCATTwoPayloads : public HashJoinEarlyBreakerBase<TABLE_PARTITION_SIZE> {
        private:
            std::vector<uint32_t> _buildColumnPipelineIds;
            uint32_t _buildBreakerPayloadColumnIdFirst; // first payload column id
            uint32_t _buildBreakerPayloadColumnIdSecond; // second payload column id
            ConciseArrayTableTwoPayloads<TABLE_PARTITION_SIZE> _hashTable;
        ...
        public:
            HashJoinEarlyBreakerCATTwoPayloads(
                ...
                _buildColumnPipelineIds(buildColumnPipelineIds), _buildBreakerPayloadColumnIdFirst(buildBreakerPayloadColumnIdFirst),
                _buildBreakerPayloadColumnIdSecond(buildBreakerPayloadColumnIdSecond) {
                // decide how many partition we need and create them
            ...
            void consumeHashTable(ConciseArrayTableTwoPayloads<TABLE_PARTITION_SIZE>& target){
    ...
    //
    template <uint32_t TABLE_PARTITION_SIZE>
    class HashJoinEarlyOperatorCATTwoPayloads : public HashJoinOperatorCATTwoPayloads<TABLE_PARTITION_SIZE> {
        private:
            std::shared_ptr<HashJoinEarlyBreakerCATTwoPayloads<TABLE_PARTITION_SIZE>> _buildBreaker;

        public:
            HashJoinEarlyOperatorCATTwoPayloads(
                const PipelineColumnsContainer<TABLE_PARTITION_SIZE>& currentPipeColumns, // since this is a flushing operator we need this information for base class
                PipelineColumnsContainer<TABLE_PARTITION_SIZE>& probeSideColumnsOnly, // necessary in expanding joins when we have to duplicate tuples
                ColumnIdMappingsContainer& columnIdMappings, // maps the columns ids of the build pipe to the columns ids of the probe pipe
                PipelineColumnsContainer<TABLE_PARTITION_SIZE>& buildPipeColumns, // column pointer needed in the probe/push function to add empty column partitions to the pushed batches
                const std::vector<uint32_t>& probeColumnPipelineIds, // indicates the column we probe on
                std::shared_ptr<HashJoinEarlyBreakerCATTwoPayloads<TABLE_PARTITION_SIZE>> buildBreaker,
                RuntimeStatistics* runtimeStatistics
            ) : HashJoinOperatorCATTwoPayloads<TABLE_PARTITION_SIZE>(
                    currentPipeColumns, probeSideColumnsOnly, columnIdMappings, buildPipeColumns, probeColumnPipelineIds, runtimeStatistics),
                _buildBreaker(buildBreaker){
